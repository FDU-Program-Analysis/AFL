...
if [ $RESUME -le $STEP ]; then
  ...
  if [ -z "$fuzzer" ]; then
    for binary in $(echo "$binaries"); do
      ...
      while ! opt-10 -dot-callgraph $binary.0.0.*.bc >/dev/null 2> $TMPDIR/step${STEP}.log ; do
    ...
    while ! opt-10 -dot-callgraph $fuzzer.0.0.*.bc >/dev/null 2> $TMPDIR/step${STEP}.log ; do
      echo -e "\e[93;1m[!]\e[0m Could not generate call graph. Repeating.."
    done
