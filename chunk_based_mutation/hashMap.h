...
// 默认哈希函数
static uint32_t defaultHashCode(HashMap hashMap, uint8_t *key);

// 默认判断键值是否相等
static Boolean defaultEqual(uint8_t *key1, uint8_t *key2);

// 默认添加键值对
static void defaultPut(HashMap hashMap, uint8_t *key, Chunk *value);

// 默认获取键对应值
static Chunk *defaultGet(HashMap hashMap, uint8_t *key);

// 默认删除键
static uint8_t *defaultRemove(HashMap hashMap, uint8_t *key);

// 默认判断键是否存在
static Boolean defaultExists(HashMap hashMap, uint8_t *key);

// 默认清空Map
static void defaultClear(HashMap hashMap);

// 重新构建
static void resetHashMap(HashMap hashMap, uint32_t listSize);
