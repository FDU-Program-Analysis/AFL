...
static int mov_read_ddts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codecpar->frame_size =
        (frame_duration_code == 0) ? 512 : (frame_duration_code == 1) ? 1024 : (frame_duration_code == 2) ? 2048 : (frame_duration_code == 3) ? 4096 : 0;
                               ...
                               int size, int distance, int flags)
{
    ...
    const size_t requested_size =
        min_size_needed > st->index_entries_allocated_size ? FFMAX(min_size_needed, 2 * st->index_entries_allocated_size) : min_size_needed;
...
static int mov_read_tfhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    frag->base_data_offset = flags & MOV_TFHD_BASE_DATA_OFFSET ? avio_rb64(pb) : flags & MOV_TFHD_DEFAULT_BASE_IS_MOOF ? frag->moof_offset : frag->implicit_offset;
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (frag_stream_info)
    {
                 ...
                 c->use_mfra_for == FF_MOV_FLAG_MFRA_DTS)
        {
            ...
            av_log(c->fc, AV_LOG_DEBUG, "found mfra time %" PRId64 ", using it for dts\n", pts);
        }
        ...
        case MKTAG('f', 't', 'y', 'p'):
                 ...
                 AV_RB64(p->buf + offset + 8) == 0))
            {
                score = FFMAX(score, AVPROBE_SCORE_EXTENSION);
                     ...
                     (AV_RL32(p->buf + offset + 8) == MKTAG('j', 'p', '2', ' ') || AV_RL32(p->buf + offset + 8) == MKTAG('j', 'p', 'x', ' ')))
            {
                score = FFMAX(score, 5);
            }
            else
...
retry:
    ...
    {
        int64_t next_dts = (sc->current_sample < st->nb_index_entries) ? st->index_entries[sc->current_sample].timestamp : st->duration;
...
static const AVOption mov_options[] = {
    ...
    {"dts", "dts", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_DTS}, 0, 0, FLAGS, "use_mfra_for"},
    {"pts", "pts", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_PTS}, 0, 0, FLAGS, "use_mfra_for"},
