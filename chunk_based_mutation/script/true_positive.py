...
chunkfuzzer = [3, 9, 0, 5, 12]
x = range(len(true_positive))

rects1 = plt.bar(x=x, height=true_positive, width=0.4, alpha=0.8, color='red', label='ChunkFuzzer识别真阳性块数')
rects2 = plt.bar(x=[i + 0.4 for i in x], height=chunkfuzzer, width=0.4, color='blue', label='ChunkFuzzer识别块数')
plt.ylim(0, 15)

plt.xticks([index + 0.2 for index in x], labels)
plt.title("ChunkFuzzer块结构识别召回率", fontproperties=myfont)
plt.legend(prop=myfont, loc="upper left")

plt.savefig("true_positive_compare.png",dpi=1080,format='png', bbox_inches='tight')
plt.show()
